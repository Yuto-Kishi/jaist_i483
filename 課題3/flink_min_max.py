...
# ── 設定 ─────────────────────────────────────────────
STUDENT_ID      = "s2410040"
INPUT_TOPIC     = f"i483-sensors-{STUDENT_ID}-.*"    # ← 正規表現が使えない場合は topics() で複数列挙
OUTPUT_TOPIC    = "i483-allsensors"                  # 出力先は自由に設定（課題3だと analytics トピックに出力する）
# ───────────────────────────────────────────────────────
...
def main():
    ...
    source = (
        ...
        .set_topics(f"i483-sensors-{STUDENT_ID}-BH1750-illumination", f"i483-sensors-{STUDENT_ID}-SCD41-co2")  # ← ここは明示的に列挙する形にします
