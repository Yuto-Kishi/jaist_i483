...
# analyse_event_time.py：イベントタイム版（課題3）
...
def main():
    ...
    # WatermarkStrategy: イベントタイム = ts部分（ミリ秒）
    stream = env.from_source(
        source,
        WatermarkStrategy
            .for_monotonous_timestamps()
            .with_timestamp_assigner(lambda raw, _: int(raw.split(",")[1])),
        "kafka_source"
    )
    ...
    stats.sink_to(sink)
    env.execute("EventTime All-Sensor Stats")
